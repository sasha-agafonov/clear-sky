...
static QImage toGL(const QImage& img)
{
    return img.convertToFormat(QImage::Format_RGBA8888).mirrored();
}

world_widget::world_widget(QSurfaceFormat const& fmt, QWidget* parent)
    : QOpenGLWidget(parent),
    hViewAng(260), vViewAng(41), zoom(0.1f), time(0),
    locked(false), manouvering(false), smooth(false), turboOff(false),
    paused(false), cameraMovement(false),
    trajectoryRad(40), cloudsAmount(10), cloudSpawnRate(1),
    aeroStyle(0), cloudStyle(0), positionAng(0.0), arcLen(0.0), rollAng(0.0)
{
    setFormat(fmt);

    trail_ = new contrail(0., 0., 0., cloudsAmount);
    shape2_ = new shape();
    aero_ = new aero(2);
    turbo_ = new turbo();
    house_ = new house();
...
void world_widget ::initializeGL()
{
...
void world_widget ::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
...
void world_widget :: updateHorizontalView(int ang) {
    hViewAng = ang;
    cameraMovement = true;
    this -> repaint();
}

void world_widget ::updateVerticalView(int ang)
{
    vViewAng = ang;
    cameraMovement = true;
    this -> repaint();
...
void world_widget ::updateZoom(int zoom)
{
    this -> zoom = zoom;
    this -> zoom /= 10;
    cameraMovement = true;
...
void world_widget :: changeTrajectory(int rad) {
    trajectoryRad = rad;
    trajectoryRad *= 10;
    manouvering = true;
    this -> repaint();
...
void world_widget :: changeAircraftSpeed(int speed) { aero_ -> changeAircraftSpeed(speed); }

void world_widget ::setCloudsAmount(int amount)
{
    trail_->numClouds_ = amount;
}

void world_widget :: setCloudSpawnRate(int rate) { cloudSpawnRate = 16 - rate; }

void world_widget :: doAileronRoll() {
...
void world_widget :: changeAeroStyle(int selection) {
    aeroStyle = selection;
    this -> repaint();
}

void world_widget :: changeCloudStyle(int selection) {
    cloudStyle = selection;
    this -> repaint();
}

void world_widget :: shadeSmooth() {
    if (smooth) smooth = false;
    else smooth = true;
    this -> repaint();
}

void world_widget :: shutTurbo() {
    if (turboOff) turboOff = false;
    else turboOff = true;
}

void world_widget :: pause() {
    if (paused) paused = false;
    else paused = true;
}

void world_widget :: paintGL() {
    ...
    if (cameraMovement) turbo_ -> turbo_inner(turboOff, cameraMovement);
    else turbo_ -> turbo_inner(turboOff, paused);
    glPopMatrix();

    double posX, posY;

    if (!paused && !manouvering && !cameraMovement) arcLen += aero_ -> speed;
    ...
    if (!cameraMovement) {
        if (aeroStyle == 0) aero_ -> aeroplane(&redPlastic, 0, paused);
        else if (aeroStyle == 1)
        {
            aero_->aeroplane(&silver, 1, paused);
        }
        else
        {
            aero_->aeroplane(&m200, 2, paused);
        }
    } else {
        if (aeroStyle == 0)
        {
            aero_->aeroplane(&redPlastic, 0, cameraMovement);
        }
        else if (aeroStyle == 1)
        {
            aero_->aeroplane(&silver, 1, cameraMovement);
        }
        else
        {
            aero_->aeroplane(&m200, 2, cameraMovement);
        }
    }
    ...
    trail_ -> afterEffect(trajectoryRad * sin((arcLen)/ (2 * PI * (trajectoryRad ))) + unitY * 20, trajectoryRad * cos((arcLen) / (2 * PI * (trajectoryRad ))) - unitX * 20, 30);

    // spawn a new cloud
    if (!cameraMovement && !paused && time % cloudSpawnRate == 0)
    {
        trail_->spawn(posX, posY, 30., -unitY, unitX, unitZ, aero_->speed);
    }

    // draw entire trail
    if (!cameraMovement)
    {
        trail_->drawTrail(aero_->speed, this->textures, paused, 3 - cloudStyle);
    }
    else
    {
        trail_->drawTrail(aero_->speed, this->textures, cameraMovement, 3 - cloudStyle);
    }
